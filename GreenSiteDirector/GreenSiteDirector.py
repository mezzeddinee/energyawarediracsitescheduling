...
class GreenSiteDirector(AgentModule):
    ...
    def submitPilots(self):
        ...
        # 🟢 Enrich queueDict with PUE and CI before processing, if they are not already existing
        # 🟢 Enrich each queue with PUE and CI if not already present
        for queueName, queueData in self.queueDict.items():
            params = queueData["ParametersDict"]  # already exists

            # Add PUE (Power Usage Effectiveness) if missing
            if "PUE" not in params:
                params["PUE"] = round(random.uniform(1.0, 2.0), 2)

            # Add CI (Carbon Intensity) if missing
            if "CI" not in params:
                params["CI"] = round(random.uniform(1.0, 1000.0), 2) # g/kwh
        ...
        def sort_by_efficiency(item):
            """
            Sorting key for queue entries based on energy efficiency.
            Lower PUE and CI values have higher priority.
            """
            queue_name, queue_data = item
            params = queue_data.get("ParametersDict", {})
            pue = float(params.get("PUE", 1.5))  # Default mid-range values
            ci = float(params.get("CI", 5.0))
            return (pue, ci)

        #################################################################
        def combined_efficiency(item, w_pue=0.5, w_ci=0.5):
            queue_name, queue_data = item
            params = queue_data.get("ParametersDict", {})
            ...
            # Normalize
            pue_norm = (pue - 1.0) / (2.0 - 1.0)  # 1.0–2.0 typical range
            ci_norm = ci / 800.0  # 0–800 typical range
        ...
        queueDictItems = list(self.queueDict.items())
        # Sort in place — instead of random.shuffle()
        queueDictItems.sort(key=sort_by_efficiency)
        #queueDictItems.sort(combined_efficiency,w_pue=0.2, w_ci=0.8)
